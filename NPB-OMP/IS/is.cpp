...
/********************/
INT_TYPE
*key_buff_ptr_global; /* used by full_verify to get copies of rank info */
...
INT_TYPE(*partial_verify_vals) = (INT_TYPE *)malloc(sizeof(INT_TYPE) *
                                                    (TEST_ARRAY_SIZE));
INT_TYPE **key_buff1_aptr = NULL;
#endif
...
int main(int argc, char **argv) {
  ...
  if (timer_on)
    timer_start(T_SORTING);
  full_verify();
  if (timer_on)
    timer_stop(T_SORTING);

  if (timer_on)
    timer_stop(T_TOTAL_EXECUTION);
  ...
  c_print_results(
      ...
      (char *)"keys ranked", passed_verification, (char *)NPBVERSION,
...
#pragma omp parallel for private(i, j, k, k1) schedule(dynamic)
  ...
  /* This is actual sorting. Each thread is responsible for a subset of key
   * values */
  j = num_procs;
  j = (MAX_KEY + j - 1) / j;
  k1 = j * myid;
